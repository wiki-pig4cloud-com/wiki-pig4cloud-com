---
title: "X86 默认用户 SYSDBA/SYSDBA"
---

##  ① 背景
默认情况下 , pig 仅提供的 MySQL 的数据库脚本，很多同学咨询是否能够提供 PG、国产化等等各种数据库的支持，答案： **必须支持**。

<Warning>
为了 SQL 的准确性、实时性，其他类型的数据库脚本，**需要开发者基于 PIG MYSQL 版本转换一下**，本文档以 达梦数据库为例；
...
### 2.2  安装 SQLark
SQLark 是一款面向信创应用开发者的数据库开发管理、数据库迁移 工具，用于快速查询、创建和管理不同类型的数据库系统，现已支持达梦、Oracle 和 MySQL 数据库。
...
3. 创建迁移评估
4. 链接源数据库 （MYSQL）
5. 选择待迁移的数据库 （pig/pig_config）, 迁移成功会自动创建 PIG/PIG_CONFIG 的达梦数据库
6. 注意关闭检查
