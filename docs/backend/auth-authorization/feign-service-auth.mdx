...
+ **<font style="color:#DF2A3F;">配置 feignclient 路径：demo-api/src/main/resources/META-INF/spring/org.springframework.cloud.openfeign.FeignClient.imports</font>**

```java
com.pig4cloud.pig.demo.api.feign.RemoteDemoService
```

## ① 带 Token 请求 (<font style="color:#DF2A3F;">不需要携带 FROM_IN header</font>)
...
## ② 无 Token 请求 (<font style="color:#DF2A3F;">需要携带 FROM_IN header</font>)
...
:::warning
**<font style="color:#F5222D;">很多情况下，比如定时任务。A 服务并没有 token 去请求 B 服务，pig 也对这种情况进行了兼容。类似于 A 对外暴露 API，但是又安全限制</font>**
